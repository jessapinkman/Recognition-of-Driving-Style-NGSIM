...
def LaneChangeCount(df):
    counts = df['Vehicle_ID'].value_counts()/160
    return counts
...
data_LC2 = data_LC1[-(((data_LC1['Frame_ID'].shift(-1)-data_LC1['Frame_ID'])<160) & (data_LC1['Vehicle_ID']\
                                      ==data_LC1['Vehicle_ID'].shift(-1)))]
data_LC2 = data_LC2.reset_index()
print(data_LC2.shape) #[373 rows x 21 columns]


#2222找到变道前后100帧的数据索引
index1 = list(data_LC2['index'])
for i in range(len(index1)):
    for j in range(index1[i]-79,index1[i]+81):
        index1.append(j)
del index1[:373] #此处删除的行数需要人为查看，有待改进
...
data_LC.to_csv(r'data/i-80_LC.csv',index=None,encoding='utf-8_sig')
print(data_LC.shape) #(59680, 18)
print("done")

#3333删除不属于变道车的数据
data1 = pd.read_csv('data/i-80_LC.csv')
index_V_ID = list(data_LC2['Vehicle_ID'])
# print(np.array(data1.loc[[0],['Vehicle_ID']]).tolist()[0][0])  #1773.0
# print(index_V_ID[math.floor(0 / 160)]) #向下取整
...
data1.to_csv(r'data/i-80_LC_16s.csv',index=None,encoding='utf-8_sig') #220辆车

data1 = pd.read_csv('data/i-80_LC_16s.csv')
counts = pd.DataFrame(LaneChangeCount(data1)).reset_index(drop=False)
counts.columns = ['Vehicle_ID', 'counts']
counts.to_csv(r'data/I-80_LC_counts.csv') #[220 rows x 2 columns]
# print(data1.shape) #(45120, 18)
...
for V_ID in list(set(list(data['Vehicle_ID']))):
    if list(data['Vehicle_ID']).count(V_ID) == 100:
